...
class WaveBarsTransitionState extends TransitionState<WaveBarsTransition>
  ...
  List<double>? _storedStartDelays; // Store wave delays for consistent exit
  ...
  void _setupAnimations(double maxWidth, double maxHeight) {
    ...
    // 2. Use stored wave delays for exit, or generate new ones for entrance
    final List<double> startDelays;
    if (_isExiting && _storedStartDelays != null) {
      startDelays = _storedStartDelays!;
    } else {
      startDelays = [0.0];
      for (int i = 1; i < _barCount; i++) {
        final prevStart = startDelays.last;
        startDelays.add(prevStart + _random.nextDouble() * 20);
      }
      if (!_isExiting) {
        _storedStartDelays = List.from(startDelays); // Store for exit phase
      }
    }
    ...
    for (int i = 0; i < _barCount; i++) {
      endTimes.add(startDelays[i] + durations[i]);
    }

    final double totalDuration = endTimes.reduce(max);
